...
const InboxComponent = ({
  ...
  return (
    <div
      onClick={handleCurThread}
      className={`${textColor} cursor-pointer w-full max-w-md flex flex-col justify-center border-b ${borderClr} p-4`}
    >
      ...
      <div className="flex items-center mt-4">
        <button
          className={`flex justify-center items-center ${bgColor} py-1 px-4 rounded-2xl mr-4 inline`}
        >
          <GoDotFill />
          <span className="ml-1 text-xs">Interested</span>
        </button>
        <button
          className={`flex justify-center items-center ${bgColor} py-1 px-2 rounded-2xl`}
        >
          <IoIosSend />
          <span className="ml-1 text-xs" style={{ whiteSpace: "nowrap" }}>
            Campaign Name
          </span>
        </button>
      </div>
