...
const Header = () => {
  ...
  return (
    <div
      className={`flex justify-between items-center h-16 w-full ${bgColor} ${textColor} border-b ${borderClr}`}
    >
      <h1 className="ml-5 font-bold text-l">Onebox</h1>
      <div className="flex items-center justify-center">
        <button
          className={`mr-6 flex items-center justify-between ${btnBgClr} border ${borderClr} rounded-full p-1`}
          onClick={handleTheme}
        >
          {theme === "dark" ? (
            <>
